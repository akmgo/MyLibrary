    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            HStack(alignment: .top, spacing: 40) {
                ...
                .frame(maxWidth: .infinity).frame(height: 266)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    .buttonStyle(.plain).frame(width: 160, height: 240)
...
private struct MetadataDivider: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Rectangle().fill(LinearGradient(colors: [Color.clear, isDark ? Color.twIndigo500.opacity(0.6) : Color.twIndigo400], startPoint: .leading, endPoint: .trailing)).frame(height: 1)
            HStack(spacing: 5) {
                ...
                Image(systemName: "chevron.right").font(.system(size: 8, weight: .black)).foregroundColor(isDark ? .twSlate500 : .twSlate400)
            }
            ZStack {
                Circle().stroke(LinearGradient(colors: [.twIndigo500, .twSky300, .purple], startPoint: .topLeading, endPoint: .bottomTrailing), lineWidth: 1.5)
                    .frame(width: 24, height: 24)
                    .rotationEffect(.degrees(isAnimating ? 360 : 0))
                    .shadow(color: .twIndigo500.opacity(isDark ? 0.6 : 0.3), radius: 6)
                Image(systemName: "rhombus.fill").font(.system(size: 10)).foregroundStyle(LinearGradient(colors: [.twSky300, .twIndigo500], startPoint: .top, endPoint: .bottom))
                Image(systemName: "sparkle").font(.system(size: 8, weight: .bold)).foregroundColor(.purple).offset(x: 10, y: -10).scaleEffect(isAnimating ? 1.1 : 0.9)
            }.padding(.horizontal, 4)
            HStack(spacing: 5) {
                Image(systemName: "chevron.left").font(.system(size: 8, weight: .black)).foregroundColor(isDark ? .twSlate500 : .twSlate400)
                Circle().fill(isDark ? Color.twPurple400 : Color.twPurple400).frame(width: 4, height: 4)
                Circle().fill(isDark ? Color.twSlate600 : Color.twSlate300).frame(width: 3, height: 3)
            }
            Rectangle().fill(LinearGradient(colors: [isDark ? Color.twPurple500.opacity(0.6) : Color.twPurple400, Color.clear], startPoint: .leading, endPoint: .trailing)).frame(height: 1)
        }
        .padding(.vertical, 8)
        .onAppear { withAnimation(.linear(duration: 8).repeatForever(autoreverses: false)) { isAnimating = true } }
    }
}

private struct DashedDropzoneView: View {
    var isDark: Bool
    var body: some View {
        ZStack {
            // ✨ 替换为 AppleCurve 全局小圆角 (16)
